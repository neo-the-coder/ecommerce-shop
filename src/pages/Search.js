  ...
  return (
    <div className="outline flex items-start">
      ...
      <section className="w-full">
        <h1 className="mt-5 mb-8 pb-2 border-b border-b-neutral text-xl text-black uppercase">{`${searchResult[query]?.total || 0} Result${searchResult[query]?.total > 1 ? 's' : ''} for `}<span className="normal-case">{`'${query}'`}</span></h1>
        <div>
          search result extra
        </div>
